function DS = get_hycom_nudge(Mobj, nudge_nodes, meta_data)
...
%% Data specification
if nargin < 3
    meta_data.src_file = 'schism-toolbox\data\hycom\W117E127S33N41_****.mat';   % the source files
    meta_data.src_time = unique(dateshift(Mobj.time, 'start', 'days')); % timestamps of the boundary data
    meta_data.raw_list = {'temp', 'salt'};  % original variable name in the files
    meta_data.new_list = {'temp', 'salt'}; % standard variable name for output
...
if isnumeric(nudge_nodes); nudge_nodes = repmat({sort(nudge_nodes(:))}, [nVars, 1]); end

lon_obc = Mobj.lon(nudge_nodes{1}); lat_obc = Mobj.lat(nudge_nodes{1});
bbox = [min(lon_obc) max(lon_obc) min(lat_obc) max(lat_obc)];
%% Pre-load grid info an subregion index
...
for iTime = 1:nt
    ...
    if iTime==1
        sz = info.bytes;
        C = load(filepath, 'lon', 'lat', 'depth');
    end
    if abs(info.bytes-sz)/sz > 0.10  % may use different coordinates
        sz = info.bytes;
        C = load(filepath, 'lon', 'lat', 'depth');
    end
    [ind_x, ind_y, lon_reg, lat_reg] = sub_region(C.lon, C.lat, bbox, 2);
...
for iVar = 1:nVars
    ...
    tmp_nodes = nudge_nodes{iVar};
    lon_obc = Mobj.lon(tmp_nodes); lat_obc = Mobj.lat(tmp_nodes);
    nps = numel(tmp_nodes);

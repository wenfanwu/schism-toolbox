function shapiro_val = gen_slope_filter2(Mobj, slope_list, val_max, disp_flag)
...
if val_max>0.5
    error('The hdif_max can not be greater than 0.5 or less than 0')
end
%% Calculate
...
for i=1:ne
    ...
    for m=1:(i34(i)-2) % split into tri
        ...
        slx=nansum(dp(elnode(nwild(1:3),i)).*dldxy(1,:)');
        sly=nansum(dp(elnode(nwild(1:3),i)).*dldxy(2,:)');
